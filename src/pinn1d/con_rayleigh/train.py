# src/pinn1d/con_rayleigh/train.py
import os
import math
import json
import numpy as np
...
def run_one_mode_learnE(n: int, cfg: dict):
    ...
    # comportamiento sin ruido (baseline Rayleigh), por lo que esta función
    # sirve para ambos experimentos.
    ...
    def apply_noise(x):
        ...
        # El cálculo de integrales en losses.py usa la regla del trapecio,
        # que asume x ordenado. Hay que reordenar tras perturbar.
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        optimizer.zero_grad()
        L, LPDE, Lnorm, integral, E = compute_losses(net, x_step, lam)
        L.backward()
    ...
    metrics = {
        ...
        "noise_resample_every_epoch": NOISE_RESAMPLE,
    }
