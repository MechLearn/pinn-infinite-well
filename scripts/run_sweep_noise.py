...
Soporta ambas formulaciones vía --formulation:
    sin_rayleigh  -> energía como parámetro libre (alpha)
    con_rayleigh  -> energía vía cociente de Rayleigh
"""
...
def parse_args():
    ...
    p.add_argument("--formulation", type=str, required=True,
                   choices=["sin_rayleigh", "con_rayleigh"])
...
def main():
    ...
    train_module = importlib.import_module(f"pinn1d.{args.formulation}.train")
    run_one_mode_learnE = train_module.run_one_mode_learnE

    total = len(args.arch) * len(args.sigmas) * len(args.seeds) * (args.n_max - args.n_min + 1)
    done = skipped = 0
    t0 = time.time()

    print(f"=== OE2: barrido de ruido en colocación ({args.formulation}) ===")
        ...
        for sigma in args.sigmas:
            ...
            for seed in args.seeds:
                for n in range(args.n_min, args.n_max + 1):
                    ...
                    cfg["save_dir"] = os.path.join(base_cfg["save_dir"], args.formulation)
    ...
    print(f"\n✅ Completado. Corridas nuevas: {done} | saltadas: {skipped} | tiempo: {elapsed/60:.1f} min")
